...
const { memoryRule } = require('./rules');

const getNodeStats = async (category) => {
  ...
  if (nodeIds.length > 0) {
    nodeIds.forEach((nodeId, index) => {
      ...
      nodeResult.memory.heap_used_percent = nodeStats.nodes[nodeId].jvm.mem.heap_used_percent;
      nodeResult.memory.heap_used_in_bytes = nodeStats.nodes[nodeId].jvm.mem.heap_used_in_bytes;
      nodeResult.memory.heap_committed_in_bytes = nodeStats.nodes[nodeId].jvm.mem.heap_committed_in_bytes;
